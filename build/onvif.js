...
class Onvif extends events_1.EventEmitter {
    ...
    constructor(options) {
        ...
        this.media2 = new media2_1.Media2(this);
    ...
    async rawRequest(options) {
        return new Promise((resolve, reject) => {
            ...
            requestOptions.headers = {
                ...
                'Content-Length': buffer_1.Buffer.byteLength(options.body, 'utf8').toString(),
                charset: 'utf-8',
            };
    ...
    digestAuth(wwwAuthenticate, path) {
    ...
    setupSystemDateAndTime(data) {
        const { systemDateAndTime } = (0, utils_1.linerase)(data).getSystemDateAndTimeResponse;
    ...
    async setSystemDateAndTime(options) {
        ...
        const body = '<SetSystemDateAndTime xmlns="http://www.onvif.org/ver10/device/wsdl">'
            ...
            + `<DaylightSavings>${!!options.daylightSavings}</DaylightSavings>${options.timezone !== undefined || options.timeZone?.TZ !== undefined
                ? '<TimeZone>'
        ...
        if ((0, utils_1.linerase)(data).setSystemDateAndTimeResponse.length !== 0) {
    ...
    async getActiveSources() {
        this.media.videoSources.forEach(({ token: videoSrcToken }, idx) => {
            ...
            const appropriateProfiles = this.media.profiles.filter((profile) => (profile.videoSourceConfiguration?.sourceToken === videoSrcToken) && (profile.videoEncoderConfiguration !== undefined));
