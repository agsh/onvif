...
/**
 * Parse an ONVIF service URL (same shape as legacy `url.parse` for Cam options).
 * @param {string} href
 * @returns {{ hostname: string, port?: number, path: string }}
 */
function parseServiceUrl(href) {
			...
			if (err || !data[0].probeMatches) {
				...
				if (!cams[camAddr]) {
					...
					if (options.resolve !== false) {
						...
						var camUris = data.probeMatches.probeMatch.XAddrs.split(' ').filter(Boolean).map(parseServiceUrl),
