...
Cam.prototype.connect = function(callback) {
	...
	this.getSystemDateAndTime(function(err, date, xml) {
		...
		self.getCapabilities(function(err, data, xml) {
			if (err) {
				callback.call(self, err, null, xml);
			} else {
				var upstartFunctions = [];

				// Profile S
				if (data && data.media && data.media.XAddr) {
					upstartFunctions.push(self.getProfiles);
					upstartFunctions.push(self.getVideoSources);
				}
	...
	this._request({
		...
		if (!err) {
			var dt = linerase(data[0]['getSystemDateAndTimeResponse'][0]['systemDateAndTime'][0]['UTCDateTime'][0])
				, time = new Date(dt.date.year, dt.date.month - 1, dt.date.day, dt.time.hour, dt.time.minute, dt.time.second)
				;
			if (!this.timeShift) {
	...
	this._request({
		...
		if (!err) {
			...
			if ((this.uri['replay']) && !this.uri['recording']) {
				var temp_recorder_xaddr = this.uri['replay'].href.replace('replay','recording');
				console.warn("WARNING: Adding " + temp_recorder_xaddr + " for bad Profile G device");
				this.uri['recording'] = url.parse(temp_recorder_xaddr);
			}
		}
		if (callback) {
