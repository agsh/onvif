 ...
 * @param {object} [time]
 * @param {Cam~getSystemDateAndTimeCallback} callback
 */
Cam.prototype.setSystemDateAndTime = function(time, callback) {
	this._request({
		...
		if (err || linerase(data).setSystemDateAndTimeResponse !== '') {
			return callback(linerase(data).setSystemDateAndTimeResponse !== '' ? new Error('Wrong `SetScopes` response') : err, data, xml);
		}
