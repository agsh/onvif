...
const http = require('http'),
	https = require('https'),
	httpAuth = require('http-auth-client'),
...
var Cam = function(options, callback) {
	...
	// Try digest authentication but fall back to WS-Security if needed
	this.useWsSecurity = false;
...
Cam.prototype._request = function(options, callback) {
	...
	if (options.action === undefined || options.action === null) {
		...
		parseString(options.body, xml2jsOptions, (err, result) => {
			...
			this._requestPart2.call(this, options, null, callback);
		});
	} else {
		this._requestPart2.call(this, options, null, callback);
	}
...
Cam.prototype._requestPart2 = function(options, authHeader, callback) {
	var _this = this;
	...
	var req = httpLib.request(reqOptions, function(res) {
		...
		if (statusCode == 401 && authHeader == null) {
			// Parse the WWW-Authentication Header sent by the Camera
			// then make a 2nd request with an Authorization header

			// parse response header and make a new HTTP Header
			let challenges = httpAuth.parseHeaders(res.headers['www-authenticate']);
			let auth = httpAuth.create(challenges);
			auth.credentials(_this.username, _this.password);
			let newHeader = auth.authorization(reqOptions.method, reqOptions.path);

			req.destroy(); // abort current request
 
			return _this._requestPart2.call(_this, options, newHeader, callback);
		}
		if (statusCode == 401 && authHeader != null) {
			// the credentials are invalid. Continue on to read in any XML message and return XML and statusCode
		}
...
Cam.prototype._envelopeHeader = function(openHeader) {
	var header = '<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:a="http://www.w3.org/2005/08/addressing">' +
        '<s:Header>';
	// Only insert Security if there is a username and password and we are in ws-security mode
	if (this.username && this.password && this.useWsSecurity) {
