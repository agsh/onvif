...
Cam.prototype.getHostname = function(callback) {
	this._request({
		...
		if (!err) {
			this.hostname = linerase(data).getHostnameResponse;
		}
		if (callback) {
			callback.call(this, err, this.hostname, xml);
		}
	}.bind(this));
};

/**
 * @callback Cam~getScopes
 * @recieve name
 */
Cam.prototype.getScopes = function(callback) {
	this._request({
		...
		if (!err) {
			this.scopes = linerase(data).getScopesResponse;
		}
		if (callback) {
 ...
 * @recieve name
 */
Cam.prototype.setScopes = function(data, callback) {
	var scopes = [];
	Object.keys(data).forEach(function(key) {
		scopes += '<Scopes>' + data[key] + '</Scopes>';
	});
	this._request({
		body: this._envelopeHeader() +
		...
		if (!err) {
			this.scopes = linerase(data).setScopesResponse;
		}
		if (callback) {
			callback.call(this, err, this.scopes, xml);
		}
