 ...
 * @param {boolean} [options.daylightSavings]
 * @patam {string} [options.timezone]
 * @param {Cam~DateTimeCallback} callback
 */
Cam.prototype.setSystemDateAndTime = function(options, callback) {
	if ([undefined, 'Manual', 'NTP'].indexOf(options.dateTimeType) === -1) {
	...
	this._request({
		...
		'<SetSystemDateAndTime xmlns="http://www.onvif.org/ver10/device/wsdl">' +
			...
			'<UTCDateTime>' + 
				'<Time xmlns="http://www.onvif.org/ver10/schema">' +
					'<Hour>' + options.dateTime.getHours() + '</Hour>' +
					'<Minute>' + options.dateTime.getMinutes() + '</Minute>' +
					'<Second>' + options.dateTime.getSeconds() + '</Second>' +
				'</Time>' +
				'<Date xmlns="http://www.onvif.org/ver10/schema">' +
					'<Year>' + options.dateTime.getFullYear() + '</Year>' +
					'<Month>' + (options.dateTime.getMonth() + 1) + '</Month>' +
					'<Day>' + options.dateTime.getDate() + '</Day>' +
				'</Date>' +
			'</UTCDateTime>' +
		...
		if (err || linerase(data).setSystemDateAndTimeResponse !== '') {
			return callback(linerase(data).setSystemDateAndTimeResponse !== '' ? new Error('Wrong `SetSystemDateAndTime` response') : err, data, xml);
		}
