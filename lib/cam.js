 ...
 * @property {?Error} error
 * @property {NTPInformation} NTP information object of current device's NTP manual
 * @property {string} xml Raw SOAP response
 */
...
/**
 * @typedef {object} Cam~NTP
 * @property {boolean} fromDHCP
 ...
 * Set the device NTP
 * @param {object} options
 * @param {bollean} [options.fromDHCP]
 * @param {string} [options.type] ( IPv4 | IPv6 | DNS )
 * @patam {string} [options.IPv4Address]
 * @patam {string} [options.IPv6Address]
 * @patam {string} [options.DNSname]
 * @param {Cam~NTPCallback} callback
 */
Cam.prototype.setNTP = function(options, callback) {
	this._request({
		...
		'<SetNTP xmlns="http://www.onvif.org/ver10/device/wsdl">' +
			...
			'<NTPManual>' +
				'<Type xmlns="http://www.onvif.org/ver10/schema">' +
					options.type +
				'</Type>' +
				(options.IPv4Address !== undefined ? 
				'<IPv4Address xmlns="http://www.onvif.org/ver10/schema">'+
					options.IPv4Address +
				'</IPv4Address>' : '') +
				(options.IPv6Address !== undefined ? 
				'<IPv6Address xmlns="http://www.onvif.org/ver10/schema">'+
					options.IPv6Address +
				'</IPv6Address>' : '') +
				(options.DNSname !== undefined ? 
				'<DNSname xmlns="http://www.onvif.org/ver10/schema">'+
					options.DNSname +
				'</DNSname>' : '') +
			'</NTPManual>' +
		'</SetNTP>' +
		...
		if (err || linerase(data).setNTPResponse !== '') {
			return callback.call(this, linerase(data).setNTPResponse !== ''
				? new Error('Wrong `SetNTP` response')
				: err, data, xml);
		}
		//get new system time from device
		this.getNTP(callback);
	}.bind(this));
};
