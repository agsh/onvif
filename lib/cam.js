...
Cam.prototype.digestAuth = function(wwwAuthenticate, reqOptions) {
	...
	let ha1;
	// If the algorithm is not supported then crpto.createHash will throw. Eg SHA-256 is not supported on older copies of NodeJS unless using an external library
	try {
		ha1 = crypto.createHash(algorithm);
	} catch (err) {
		// cannot create hash
