...
Cam.prototype._requestPart2 = function(options, callback) {
	...
	const req = httpLib.request(reqOptions, function(res) {
		...
		if (statusCode === 401 && reqOptions.headers['Authorization'] !== undefined) {
			...
			callback('Digest authorization failed', '', '', statusCode);
			return;
		}
...
Cam.prototype.createHash = function(algorithm) {
	...
	try {
		...
		try {
			...
			if (algorithmLower == "sha-256" || algorithmLower == "sha256") {
				hash = shajs('sha256');
			} else if (algorithmLower == "sha-512" || algorithmLower == "sha512") {
				hash = shajs('sha512');
			} else {
...
/**
 * Retourne le service et le namespace XML selon la version Media (1 ou 2).
 * @returns {{ service: string, xmlns: string }}
 * @private
 */
Cam.prototype._getMediaServiceInfo = function() {
	return {
		service: this.media2Support ? 'media2' : 'media',
		xmlns: this.media2Support ? 'http://www.onvif.org/ver20/media/wsdl' : 'http://www.onvif.org/ver10/media/wsdl'
	};
};

/**
 * Construit le body SOAP pour les requêtes Get* Media avec options optionnelles (configurationToken, profileToken).
 * @param {string} actionName - Ex: 'GetVideoSourceConfigurations'
 * @param {object} [options] - { configurationToken?, profileToken? }
 * @returns {string} Fragment XML (sans envelope)
 * @private
 */
Cam.prototype._buildGetConfigRequestBody = function(actionName, options) {
	const { xmlns } = this._getMediaServiceInfo();
	const hasOpts = options && typeof options === 'object' && (options.configurationToken || options.profileToken);
	if (!hasOpts) {
		return `<${actionName} xmlns="${xmlns}"/>`;
	}
	const parts = [
	...
	return `<${actionName} xmlns="${xmlns}">${parts.join('')}</${actionName}>`;
};
