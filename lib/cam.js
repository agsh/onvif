...
const http = require('http'),
	...
	emptyFn = function() {};
const { getDigestHeaders } = require('./utils');
 ...
 * @property {boolean} [useWSSecurity] Use WS-Security SOAP headers (set to false if you want to force use Digest authentication)
...
Cam.prototype.connect = function(callback) {
	...
	this.getSystemDateAndTime(function(err, date, xml) {
		...
		function callUpstartFunctions() {
			...
			// Profile S / Media1, or Media2-only (Profile T) devices
			if (this.uri && (this.uri.media || (this.media2Support && this.uri.media2))) {
			...
			if (count > 0) {
				upstartFunctions.forEach(function(fun) {
					fun.call(this, function(err) {
						if (err) {
							mediaErrors.push(err);
						}
						if (!--count) {
							// Axis A1601 / doorcontrol: Media may be advertised but GetProfiles /
							// GetVideoSources return "Optional action not implemented".
							// Both calls are needed for active sources; if either fails, drop media state.
							if (mediaErrors.length) {
								this.profiles = [];
								this.videoSources = [];
								this.emit('warning', mediaErrors[0].message || String(mediaErrors[0]));
							}
...
Cam.prototype._requestPart2 = function(options, callback) {
	...
	const req = httpLib.request(reqOptions, function(res) {
		...
		// From the Core Spec, Section 5.9.1
		// In Digest mode, the camera will return 401 to ask for Digest Authentication, or to report that the www-authenticate header was not accepted
		// If WS-Security is used, the camera will return a 400 error to report that the ws-security header was not accepted with a SOAP fault of ter:NotAuthorized
		if (statusCode === 401 && reqOptions.headers['Authorization'] !== undefined) {
			// We sent a request with the Digest Authorization Header but it was rejected with 401. Our username or password are incorrect.
			callbackExecuted = true;
			callback(new Error(`Digest authentication failed ${statusCode}`));
			return;
		}
		if (statusCode === 401 && wwwAuthenticate !== undefined) {
			...
			//]
			const wwwAuthenticateArray = getDigestHeaders(res.rawHeaders);
			if (wwwAuthenticateArray.length === 0) {
				// Avoid racing with req 'error' from response.destroy() below
				callbackExecuted = true;
				res.destroy();
				callback(new Error(`Digest authentication headers not found ${statusCode}`));
				return;
			}
			try {
				options.headers.Authorization = _this.digestAuth(wwwAuthenticateArray, reqOptions);
				// Mark done before destroy so a concurrent ECONNRESET does not fire callback
				callbackExecuted = true;
				res.destroy();
 ...
 * @param {string[]} wwwAuthenticateArray
...
Cam.prototype.getActiveSources = function() {
	// Profile C / doorcontrol / failed media discovery — nothing to resolve
	if (!this.videoSources || (Array.isArray(this.videoSources) && this.videoSources.length === 0)) {
		return;
	}
	if (!this.profiles) {
		this.profiles = [];
