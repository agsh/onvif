...
module.exports = function(Cam) {
	...
	Cam.prototype.pullMessages = function(options, callback) {
		...
		sendXml += '</s:Header>' +
			'<s:Body xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">' +
				'<PullMessages xmlns="http://www.onvif.org/ver10/events/wsdl">' +
					'<Timeout>' + (options.pullTimeout || 'PT1M') + '</Timeout>' + // ONVIF Spec says cameras must support 1 Minute wait times. Ensure network socket has a replyTimeout that is larger that 1 minute
		...
		this._request({
			url: urlAddress,
			body: sendXml,
			replyTimeout: (options.replyTimeout || 80*1000) // 80 seconds - ensures the socket does not get closed too early while the camera has up to 1 minute to reply
		}, function(err, res, xml) {
			if (!err) {
	...
	Cam.prototype._eventRequest = function() {
		if (this.listeners('event').length) { // check for event listeners, if zero, stop pulling
			this.emit('info', 'Cam::_eventRequest(): at least 1 listener on event <event>. Check subscriptions...');
			...
			if (!this.events.subscription || !this.events.terminationTime || (Date.now() > this.events.terminationTime)) {
				...
				this.emit('info', 'Cam::_eventRequest(): start subscription and event pull...');
			...
			this.emit('info', 'Cam::_eventRequest(): no listeners on event <event>. Unsubscribe...');
			delete this.events.terminationTime;
			this.unsubscribe();
		}
	};
	...
	Cam.prototype._eventPull = function() {
		...
		let nListeners = this.listeners('event').length;
		let subscription = this.events.subscription;
		let infoSub = subscription ? 'done' : 'NONE';
		let pullInfo = `Cam::_eventPull(): ${nListeners} listeners on event <event>; subscription: ${infoSub}`;
		this.emit('info', pullInfo);
		...
		if (this.listeners('event').length && this.events.subscription) { // check for event listeners, if zero, or no subscription then stop pulling
			...
			this.emit('info', 'Cam::_eventPull(): no listener on event <event>. Unsubscribe...');
			delete this.events.terminationTime;
