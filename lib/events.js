...
module.exports = function(Cam) {
	...
	function _terminationTime(response) {
		...
		console.log("Events: Termintion Time is " + result);
		return result;
	}
			...
			this.pullMessages({
				...
				if (!err) {
					...
					this.events.terminationTime = _terminationTime(data); // Axis does not increment the termination time. Use RENEW
					                                                      // Vista returns a termination time with the time now (ie we have expired).even if there was still time left over. Use RENEW
					...
					this.unsubscribe({}, function(err,data,xml) {
