			...
			if (err || !data[0].probeMatches) {
				...
				if (!cams[camAddr]) {
					...
					if (options.resolve !== false) {
						// data.probeMatches.probeMatch.XAddrs can contain both IPv4 and IPv6
						// addresses, delimited by a space. IPv4 is always first.
						var addresses = data.probeMatches.probeMatch.XAddrs.split(' ');
						// TODO: For now, we will ignore IPv6 paths
						var camUri = url.parse(addresses[0]);
						cam = new Cam({
