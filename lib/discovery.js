...
Discovery.probe = function(options, callback) {
	...
	const listener = function(msg, rinfo) {
		setImmediate(() => {
			parseSOAPString(msg.toString(), function(err, data, xml, _statusCode) {
				...
				// domotz BT-6558: guard against malformed probe responses that throw on property access
				try {
	...
	const bindingCallback = function(err) {
		if (err) {
			...
			// domotz BT-990: allow unicast/custom discovery address (Docker bridge drops multicast)
			socket.send(request, 0, request.length, 3702, options.customDiscoveryAddress || '239.255.255.250');
		}
	};
