	...
	this._request({
		...
		if (!err) {
			...
			data[0]['getConfigurationsResponse'].forEach(function(configuration) {
				let pTZConfiguration = configuration['PTZConfiguration'][0];
				...
				if ('defaultPTZSpeed' in pTZConfiguration) {
					let defaultPTZSpeed = pTZConfiguration['defaultPTZSpeed'][0];
				...
				if ('panTiltLimits' in pTZConfiguration) {
					...
					let panTiltLimits = pTZConfiguration['panTiltLimits'][0]['range'][0];
				...
				if ('zoomLimits' in pTZConfiguration) {
					...
					let zoomLimits = pTZConfiguration['zoomLimits'][0]['range'][0];
