			...
			if (!err) {
				...
				data[0]['getConfigurationsResponse'].forEach(function(configuration) {
					...
					configuration['PTZConfiguration'].forEach(function(pTZConfiguration) {
						...
						/**
						 * Get extended properties according to the GetConfigurations 
						 * Extensions are optional and should be handled by the caller
						 * https://www.onvif.org/onvif/ver20/ptz/wsdl/ptz.wsdl 
						 * Extensions should include PTControlDirection  
						 */

						if ('extension' in pTZConfiguration) {
							const extension = pTZConfiguration['extension'][0];
							configurations[name].extension = {};
							if ('PTControlDirection' in extension) {
								const PTControlDirection = extension['PTControlDirection'][0];
								configurations[name].extension.PTControlDirection = {};
								if ('reverse' in PTControlDirection){
									configurations[name].extension.PTControlDirection.reverse = {};
									if ('mode' in PTControlDirection['reverse'][0]) {
										const mode = PTControlDirection['reverse'][0]['mode'];
										configurations[name].extension.PTControlDirection.reverse.mode = mode[0];
									}
								}
								if ('eflip' in PTControlDirection && 'mode' in PTControlDirection['eflip'][0]) {
									configurations[name].extension.PTControlDirection.eflip = PTControlDirection['eflip'][0]['mode'];
								}
							}
						}
					});
