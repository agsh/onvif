		...
		res.on('end', function() {
			...
			parseString(xml, function(err, result) {
				if (!result || !result['SOAP-ENV:Envelope'] || !result['SOAP-ENV:Envelope']['SOAP-ENV:Body']) {
                    ...
                    if (!err && result['SOAP-ENV:Envelope']['SOAP-ENV:Body'][0]['SOAP-ENV:Fault']) {
                        err = new Error(
                            'ONVIF SOAP Fault: ' +
                                result['SOAP-ENV:Envelope']['SOAP-ENV:Body'][0]['SOAP-ENV:Fault'][0]['SOAP-ENV:Reason'][0]['SOAP-ENV:Text'][0]._
                        );
                    }
