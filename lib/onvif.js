...
Cam.prototype._request = function(options, callback) {
	...
	var req = http.request(reqOptions, function(res) {
		...
		res.on('end', function() {
			...
			parseString(xml, {
				tagNameProcessors: [processors.stripPrefix, function(str) {
 ...
 * @private
 */
Cam.prototype._speed = function(speed) {
	return (speed
			?
				'<Speed>' +
				(speed.x && speed.y ? '<PanTilt x="' + speed.x + '" y="' + speed.y + '" space="http://www.onvif.org/ver10/tptz/PanTiltSpaces/PositionGenericSpace" xmlns="http://www.onvif.org/ver10/schema"/>' : '') +
				(speed.zoom ? '<Zoom x="' + speed.zoom + '" space="http://www.onvif.org/ver10/tptz/ZoomSpaces/PositionGenericSpace" xmlns="http://www.onvif.org/ver10/schema"/>' : '') +
				'</Speed>'
			: '');
};
