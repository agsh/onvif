...
Cam.prototype.getDeviceInformation = function(callback) {
	this._request({
		...
		this.deviceInformation = _linerase(data);
		if (callback) {
...
Cam.prototype.getStreamUri = function(options, callback) {
	this._request({
		...
		this.streamUri = _linerase(data);
		if (callback) {
			callback(null, this.streamUri);
		}
	}.bind(this));
};
...
var cam = new Cam({hostname: 'localhost', port: 10101, username: 'admin', password: '9999'}, function(err) {
	...
	//this.getStreamUri({}, function(err,d){console.log(JSON.stringify(d));});
});
