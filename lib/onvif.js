...
Cam.prototype.getSystemDateAndTime = function(callback) {
	this._request({
		...
		callback.call(this, err, err ? null : new Date(dt.date.year, dt.date.month, dt.date.day, dt.time.hour, dt.time.minute, dt.time.second));
	});
};
...
Cam.prototype.getCapabilities = function(callback) {
	this._request({
		...
		if (callback) {
			callback.call(this, err, this.capabilities);
		}
	}.bind(this));
};
...
Cam.prototype.getServiceCapabilities = function(callback) {
	this._request({
		...
		if (callback) {
			callback.call(this, err, this.serviceCapabilities);
		}
	}.bind(this));
};
...
Cam.prototype.getVideoSources = function(callback) {
	...
	this._request({
		...
		if (callback) {
			callback.call(this, err, this.videoSources);
		}
	}.bind(this));
};
...
Cam.prototype.getProfiles = function(callback) {
	this._request({
		...
		if (callback) {
			callback.call(this, err, this.profiles);
		}
	}.bind(this));
};
...
Cam.prototype.getServices = function(callback) {
	this._request({
		...
		if (callback) {
			callback.call(this, err, this.services);
		}
	}.bind(this));
};
...
Cam.prototype.getDeviceInformation = function(callback) {
	this._request({
		...
		if (callback) {
			callback.call(this, err, this.deviceInformation);
		}
	}.bind(this));
};
...
Cam.prototype.getStreamUri = function(options, callback) {
	...
	this._request({
		...
		// }
		if (callback) {
			callback.call(this, err, err ? null : _linerase(data).getStreamUriResponse.mediaUri);
		}
	}.bind(this));
};
...
Cam.prototype.getPresets = function(options, callback) {
	this._request({
		...
		if (callback) {
			callback.call(this, err, this.presets);
		}
	}.bind(this));
};
...
Cam.prototype.getStatus = function(options, callback) {
	this._request({
		...
		callback.call(this, err, err ? null : status);
	}.bind(this));
};
 ...
 */
Cam.prototype.getNodes = function() {
	this._request({
		body: this._envelopeHeader() +
			'<GetNodes xmlns="http://www.onvif.org/ver20/ptz/wsdl" />' +
			this._envelopeFooter()
	}, function(err, data) {
		console.log(_linerase(data));
		/*if (!err) {
			var res = _linerase(data).getStatusResponse.PTZStatus;
			var status = {
				position: {
...
Cam.prototype.getConfigurations = function(callback) {
	this._request({
		...
		if (callback) {
			callback.call(this, err, this.configurations);
		}
	}.bind(this));
};
...
Cam.prototype.getConfigurationOptions = function(configurationToken, callback) {
	this._request({
		...
		if (callback) {
			callback.call(this, err, configOptions);
		}
	}.bind(this));
};
