...
module.exports = function(Cam) {
	 ...
	 */
	Cam.prototype.getVideoSourceConfigurations = function(options, callback) {
		if (typeof options === 'function') {
			callback = options;
			options = undefined;
		}
		const { service } = this._getMediaServiceInfo();
		this._request({
			service: service
			, body: this._envelopeHeader() +
			this._buildGetConfigRequestBody('GetVideoSourceConfigurations', service === 'media2' && options) +
			this._envelopeFooter()
		}, function(err, data, xml) {
			if (!err) {
	...
	Cam.prototype.getVideoEncoderConfigurationOptions = function(options, callback) {
		...
		} else if (!(options && (options.configurationToken !== undefined || options.profileToken !== undefined))) {
		...
		this._request({
			service: service
			, body: this._envelopeHeader() +
			this._buildGetConfigRequestBody('GetVideoEncoderConfigurationOptions', options) +
			this._envelopeFooter()
		}, function(err, data, xml) {
			if (callback) {
	 ...
	 */
	Cam.prototype.getVideoEncoderConfigurations = function(options, callback) {
		if (typeof options === 'function') {
			callback = options;
			options = undefined;
		}
		...
		this._request({
			service: service
			, body: this._envelopeHeader() +
			this._buildGetConfigRequestBody('GetVideoEncoderConfigurations', this.media2Support && options) +
			this._envelopeFooter()
		}, function(err, data, xml) {
			if (!err) {
	...
	Cam.prototype.setVideoEncoderConfiguration = function(options, callback) {
		if (options.token === undefined && !(options.$ && options.$.token !== undefined)) {
			return callback(new Error('No video encoder configuration token is present!'));
		}
		const { service, xmlns } = this._getMediaServiceInfo();
		...
		const objectSent = `<SetVideoEncoderConfiguration xmlns="${xmlns}">` +
				...
				( options.rateControl ?
					...
					( options.rateControl.frameRateLimit !== undefined ? '<FrameRateLimit>' + options.rateControl.frameRateLimit + '</FrameRateLimit>' : '' ) +
					( options.rateControl.encodingInterval !== undefined ? '<EncodingInterval>' + options.rateControl.encodingInterval + '</EncodingInterval>' : '' ) +
					( options.rateControl.bitrateLimit !== undefined ? '<BitrateLimit>' + options.rateControl.bitrateLimit + '</BitrateLimit>' : '' ) +
				'</RateControl>' : '' ) +
				( options.MPEG4 ?
		...
		this._request({
			...
			if (!this.media2Support) {this.getVideoEncoderConfiguration(options.token || options.$.token, callback);} else {this.getVideoEncoderConfigurations(options, callback);}
		}.bind(this));
	};
	 ...
	 * @param callback
	 */
	Cam.prototype.getAudioSourceConfigurations = function(options, callback) {
		if (typeof options === 'function') {
			callback = options;
			options = undefined;
		}
		...
		this._request({
			service: service
			, body: this._envelopeHeader() +
			this._buildGetConfigRequestBody('GetAudioSourceConfigurations', options) +
			this._envelopeFooter()
		}, function(err, data, xml) {
			if (!err) {
				const response = data[0] && data[0].getAudioSourceConfigurationsResponse;
				const firstElement = Array.isArray(response) ? response[0] : response;
				const configurations = (firstElement && firstElement.configurations && Array.isArray(firstElement.configurations))
					? firstElement.configurations
					: [];
				this.audioSourceConfigurations = configurations.map(function(config) {
					return linerase(config);
				});
			}
			if (callback) {
	...
	*/
	 ...
	 */
	Cam.prototype.getProfiles = function(options, callback) {
		const { service, xmlns } = this._getMediaServiceInfo();
		...
		if (this.media2Support) {
			...
			this._request({
				service: service
				, body: this._envelopeHeader() +
					`<GetProfiles xmlns="${xmlns}">` +
					(options && typeof options === 'object' && options.profileToken ? `<Token>${options.profileToken}</Token>` : '') +
					'<Type>All</Type>' +
					'</GetProfiles>' +
					this._envelopeFooter()
			}, function(err, data, xml) {
	...
	Cam.prototype.createProfile = function(options, callback) {
		this._request({
			...
			if (callback) {
				let profile = null;
				if (!err && data) {
					const response = linerase(data).createProfileResponse;
					profile = (response && response.profile) ? response.profile : null;
				}
				callback.call(this, err, err ? null : profile, xml);
			}
		}.bind(this));
	};
   ...
   */
	Cam.prototype.createOSD = function(options, callback) {
		...
		this._request({
			service: service
			, body: this._envelopeHeader() +
              `<wsdl:CreateOSD xmlns:wsdl="${xmlns}" xmlns:sch="http://www.onvif.org/ver10/schema">
         <wsdl:OSD token="">
   ...
   */
	Cam.prototype.setOSD = function(options, callback) {
		...
		this._request({
			service: service
			, body: this._envelopeHeader() +
					`<wsdl:SetOSD xmlns:wsdl="${xmlns}" xmlns:sch="http://www.onvif.org/ver10/schema">
