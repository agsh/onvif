...
module.exports = function(Cam) {
	...
	*/
	 ...
	 * @param {string} [options.videoSourceConfigurationToken] Token of the Video Source Configuration, which has associated OSDs. Defaults to Active Source
	 * @param {string} [options.plaintext] Text to overlay
	 * @param {object|string} [options.position] UpperLeft, UpperRight, LowerLeft or LowerRight. Default LowerLeft
	 * @param {number} [options.position.x] x position of OSD, in pixels, counting from left to right
	 * @param {number} [options.position.y] y position of OSD, in pixels, counting from up to down
	 * @param {Cam~GetOSDOptionsCallback} callback
	 */
	Cam.prototype.setOSD = function(options, callback) {
		...
		this._request({
         ...
         <wsdl:OSD token="${options.OSDToken}">
            ...
            <sch:Type>Text</sch:Type>
            <sch:Position>
               <sch:Type>${typeof (options.position) === "object" ? "Custom" : options.position}</sch:Type>
