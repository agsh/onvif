...
Cam.prototype.getSnapshotUri = function(options, callback) {
	...
	this._request({
		...
		if (!err) {
			this.snapshotUri = linerase(data).getSnapshotUriResponse.mediaUri;
		}
		if (callback) {
			callback.call(this, err, this.snapshotUri, xml);
		}
	}.bind(this));
};
...
/**
 * Receive snapshot
 * For this method to work, you must have previously called the #getSnapshotUri method.
 * @returns Promise - on success { mimeType:string, prefix:string, rawImage:string }
 */
Cam.prototype.getSnapshot = function() {
	const self = this;
	return new Promise(function(resolve, reject) {
		if (!self.snapshotUri) {
			reject(new Error('No snapshotUri. Call #getSnapshotUri method before calling the #snapshot method'));
		} else {
			const snapshotUri = self.snapshotUri.uri;
			request({
					method: 'GET',
					uri: snapshotUri,
					gzip: true,
				...
				function(error, response, body) {
					if (error) {
						...
						const rawImage = Buffer.from(body, 'binary').toString('base64');
						const prefix = 'data:' + mimeType + ';base64,';
						const data = {
							// save the mimetype as it could be needed if
							// this is a server sending to a client
							// example: (res is a {response} ie: (req, res))
							// res.type(returnedData.mimeType)
							// res.send(returnedData.prefix + returnedData.rawImage)
							mimeType: mimeType,
							prefix: prefix,
							rawImage: rawImage
						};
