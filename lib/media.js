...
module.exports = function(Cam) {
	 ...
	 * Uses Media1 GetVideoSources when available. Media2 has no GetVideoSources; if Media1 is
	 * absent, empty, or fails and Media2 is supported, falls back to GetVideoSourceConfigurations
	 * and maps unique sourceToken values to video sources.
	 * @param {Cam~GetVideoSourcesCallback} [callback]
	 */
	Cam.prototype.getVideoSources = function(callback) {
		var mediaError = null;

		var finishWithMedia2 = function() {
			if (!(this.media2Support && this.uri && this.uri.media2)) {
				if (callback) {
					callback.call(this, mediaError, this.videoSources);
				}
				return;
			}
			this._request({
				...
				'<GetVideoSourceConfigurations xmlns="http://www.onvif.org/ver20/media/wsdl"/>' +
				this._envelopeFooter()
			}, function(err, data, xml) {
				if (err) {
					if (callback) {
						callback.call(this, mediaError || err, this.videoSources, xml);
					}
					return;
				}
				var configurations = [];
				try {
					configurations = data[0].getVideoSourceConfigurationsResponse[0].configurations || [];
				...
				var sources = [];
				var seen = {};
				configurations.forEach(function(raw) {
					var obj = linerase(raw);
					var token = obj.sourceToken;
					if (!token || seen[token]) {
						return;
					}
					seen[token] = true;
					var bounds = obj.bounds && obj.bounds.$ ? obj.bounds.$ : (obj.bounds || {});
					sources.push({
						$: { token: token }
						, framerate: 0
						, resolution: {
				...
				if (callback) {
					callback.call(this, null, this.videoSources, xml);
				}
			}.bind(this));
		}.bind(this);

		if (!(this.uri && this.uri.media)) {
			return finishWithMedia2();
		}
		...
		this._request({
			...
			if (!err) {
				...
				if (!Array.isArray(this.videoSources)) {
					this.videoSources = this.videoSources ? [this.videoSources] : [];
				}
	...
	Cam.prototype.setVideoEncoderConfiguration = function(options, callback) {
		...
		this._request({
			...
			`<SetVideoEncoderConfiguration xmlns="${xmlns}">` +
				...
				'</Resolution>' : '') +
				( options.quality || options.quality === 0 ? '<Quality xmlns="http://www.onvif.org/ver10/schema">' + options.quality + '</Quality>' : '' ) +
				( options.rateControl ?
				...
				( options.MPEG4 ?
					...
					( (options.MPEG4.profile || options.MPEG4.mpeg4Profile) ? '<Mpeg4Profile>' + (options.MPEG4.profile || options.MPEG4.mpeg4Profile) + '</Mpeg4Profile>' : '') +
				'</MPEG4>' : '') +
				( options.H264 ? '<H264 xmlns="http://www.onvif.org/ver10/schema">' +
					( options.H264.govLength ? '<GovLength>' + options.H264.govLength + '</GovLength>' : '' ) +
					// getVideoEncoderConfiguration returns H264.H264Profile; accept it so a read configuration can be written back
					( (options.H264.profile || options.H264.H264Profile) ? '<H264Profile>' + (options.H264.profile || options.H264.H264Profile) + '</H264Profile>' : '' ) +
				'</H264>' : '') +
				( options.multicast ?
