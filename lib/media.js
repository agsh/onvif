	...
	Cam.prototype.getVideoSources = function(callback) {
		this._request({
			...
			if (!err) {
				 ...
				 */
				// domotz NEC-1494: guard against malformed responses lacking getVideoSourcesResponse
				var linerasedVideoSources = linerase(data);
				this.videoSources = linerasedVideoSources && linerasedVideoSources.getVideoSourcesResponse ? linerasedVideoSources.getVideoSourcesResponse.videoSources : undefined;
		...
		if (this.media2Support) {
			...
			this._request({
				...
				if (!err) {
					 ...
					 */
					// domotz NEC-1494: guard against malformed responses lacking profiles
					var media2ProfilesData = data[0]['getProfilesResponse'] && data[0]['getProfilesResponse'][0] && data[0]['getProfilesResponse'][0]['profiles'];
					this.profiles = media2ProfilesData ? media2ProfilesData.map(function(profile) {
			...
			this._request({
				...
				if (!err) {
					 ...
					 */
					// domotz NEC-1494: guard against malformed responses lacking profiles
					var media1ProfilesData = data[0]['getProfilesResponse'] && data[0]['getProfilesResponse'][0] && data[0]['getProfilesResponse'][0]['profiles'];
					this.profiles = media1ProfilesData ? media1ProfilesData.map(function(profile) {
