			...
			this._request({
				...
				if (!err) {
					...
					this.profiles = data[0]['getProfilesResponse'][0]['profiles'].map(function(profile) {
						...
						if (tmp.configurations.audioOutput) {newProfile.extension.audioOutputConfiguration = tmp.configurations.audioOutput;}
						if (tmp.configurations.audioDecoder) {newProfile.extension.audioDecoderConfiguration = tmp.configurations.audioDecoder;}
